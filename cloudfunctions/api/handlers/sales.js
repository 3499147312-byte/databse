...
async function getSalesPage() {
  ...
  return {
    ...
    canSubmit: user.role === "rep" && stores.length > 0,
    ...
    sales: sales.map((sale) => {
      ...
      return {
        ...
        canCorrect: ["boss", "manager"].includes(user.role) && sale.status === "已通过" && sale.correctionStatus === "正常"
      };
    })
  };
}
...
async function approveSale(payload) {
  ...
  if (!sale || !canSeeScoped(user, sale)) fail("SALE_FORBIDDEN", "销售单不存在或不在当前权限范围。");
  const userId = userBusinessId(user);

  if (sale.status === "待主管审核") {
    if (user.role !== "supervisor" || sale.supervisorId !== userId) fail("WRONG_APPROVAL_LEVEL", "当前不是这张销售单的主管审批人。");
    await db.collection("sales").doc(sale._id).update({
      data: {
        status: "待经理审核",
        approvalTrail: command.push({ role: "supervisor", userId, actorName: user.name, time: nowIso() }),
        updatedAt: nowIso()
      }
    });
    await writeAudit(user, "主管审核销售", sale._id, "主管通过，流转到经理审核");
    return { status: "待经理审核" };
  }

  if (sale.status !== "待经理审核" || user.role !== "manager" || sale.managerId !== userId) {
    fail("WRONG_APPROVAL_LEVEL", "当前不是这张销售单的经理审批人。");
  }

  await db.runTransaction(async (transaction) => {
    const current = (await transaction.collection("sales").doc(sale._id).get()).data;
    if (current.status !== "待经理审核" || current.managerId !== userId) fail("SALE_CHANGED", "销售单状态已经变化，请刷新后重试。");
    ...
    await transaction.collection("sales").doc(current._id).update({
      data: {
        status: "已通过",
        approvedAt: nowIso(),
        approvalTrail: command.push({ role: "manager", userId, actorName: user.name, time: nowIso() }),
        updatedAt: nowIso()
      }
    });
  });
  ...
  await writeAudit(user, "经理审核销售", sale._id, "经理最终通过，库存、业绩、提成和应收生效");
  return { status: "已通过" };
}
...
async function rejectSale(payload) {
  ...
  const supervisorCanReject = user.role === "supervisor" && sale.supervisorId === id && sale.status === "待主管审核";
  const managerCanReject = user.role === "manager" && sale.managerId === id && sale.status === "待经理审核";
  if (!supervisorCanReject && !managerCanReject) fail("WRONG_APPROVAL_LEVEL", "当前不是这张销售单的审批人或状态已经变化。");
  ...
  await db.collection("sales").doc(sale._id).update({
    data: {
      ...
      rejectedBy: id,
      rejectedAt: nowIso(),
      approvalTrail: command.push({ role: user.role, userId: id, actorName: user.name, result: "驳回", reason, time: nowIso() }),
      updatedAt: nowIso()
    }
  });
  await writeAudit(user, "驳回销售日报", sale._id, reason);
  return { status };
}
